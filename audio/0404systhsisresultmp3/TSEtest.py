...
def testmodeloffline4ch(mixpath, targetpath, auxpath):
    ...
    expname ="offline_4ch_TSE_"+os.path.basename(mixpath).split('.')[0]+"_TSE_"+os.path.basename(s1_name).split('_')[-1]
...
def testmodelonline2ch(mixpath, targetpath, auxpath):
    ##模型测试 #offline  4 channel test
    ...
    expname ="online_2ch_TSE_"+os.path.basename(mixpath).split('.')[0]+"_TSE_"+os.path.basename(targetpath).split('_')[-1]
    expname = expname.split(".")[0]
    sf.write(os.path.join("projects/debug/0404systhsisaudio", expname+".wav"), est_s1_np, samplerate=16000)
    print("show extract results")
    plt.figure(figsize=(10,10))
    plt.subplot(3,1,1)
    plt.plot(mix_np[:,0])
...
if __name__=="__main__":
    mixname = "projects/debug/0404systhsisaudio/spk1m_spk2w_ov100_mix.wav"
    s1_name = "projects/debug/0404systhsisaudio/spk1m_spk2w_ov100_s1.wav"
    auxname = "projects/debug/0404systhsisaudio/spk1m_spk2w_s1aux.wav"
    #testmodeloffline4ch(mixname, s1_name, auxname)
